...
**数据源**：当月所有 day_summary 文件，从两个位置收集：
- `report/` 下当月的日期目录
...
**更新时机**：每月第一个交易日的晚报任务执行后更新（汇总到上个月为止的数据）。

**数据源**：当年各月的月度总结报告，从两个位置收集：
- `report/monthly/` 下当年的月度总结
...
**更新规则**：
- 如果不是当月第一个交易日，跳过此步骤
- 如果是当月第一个交易日，汇总当年所有已完成月份的月度总结，生成/更新年度总结
- 两个位置各存一份，内容完全相同
